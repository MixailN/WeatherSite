...
function onloadFavoriteCity(name) {
    ...
    const loader = document.getElementById("favorite-city-loader");
    const favoriteCityCard = document.getElementById("favorite-city-card");
    const liLoader = document.createElement("li");
    const list = document.getElementsByClassName("favorites")[0];
    list.appendChild(liLoader);
    liLoader.appendChild(document.importNode(loader.content, true));
    ...
    xhr.onload = function() {
        if (xhr.status !== 200) {
            ...
            console.log(favoriteCityCard.content);
            const liCity = document.createElement("li");
            liCity.classList.add("favorite-city");
...
function getFavoriteCity(name){
    ...
    let xhr = new XMLHttpRequest();
    const loader = document.getElementById("favorite-city-loader");
    const card = document.getElementById("favorite-city-card");
    const liLoader = document.createElement("li");
    const list = document.getElementsByClassName("favorites")[0];
    list.appendChild(liLoader);
    liLoader.appendChild(document.importNode(loader.content, true));
    ...
    xhr.onload = function() {
        if (xhr.status !== 200) {
            ...
            if(count == null) {
                liLoader.remove();
                card.content.querySelectorAll(".favorite-city-name")[0].textContent = `${answer.name}`;
                card.content.querySelectorAll(".favorite-temperature")[0].textContent =
                    `${Math.round(answer.main.temp)}\u00B0C`;
                card.content.querySelectorAll(".weather-icon")[0].src =
                    `http://openweathermap.org/img/wn/${answer.weather[0].icon}@2x.png`;
                ...
                const liCity = document.createElement("li");
                list.appendChild(liCity);
                liCity.appendChild(document.importNode(card.content, true));
